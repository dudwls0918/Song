...
void MusicManager::delete_song(int songId) {
    ...
    if (song != nullptr) {
        ...
        string artist = song->get_artist();
        delete song;
        cout << title << " by "<< artist<< " deleted." << endl;
    }
}
...
void MusicManager::delete_from_list(int sid, string pname) {
    ...
    cout << song_ptr->get_title() << " by " << song_ptr->get_artist() <<
        " deleted from playlist " << pname << endl;
}
...
void MusicManager::delete_list(string pname) {
    ...
    delete* it;
    playlists.erase(it);
    cout << "Playlist " << pname << " deleted"<< endl;
    
}

void MusicManager::save(string savefile) {
    auto plistit = find_playlist(string("All"));
    vector<Song*>tracks = (*plistit)->get_tracks();
    ofstream outfile(savefile);
    for (auto it = tracks.begin(); it != tracks.end(); it++) {
        string artist = (*it)->get_artist();
        string title = (*it)->get_title();
        string album = (*it)->get_album();
        string mv_url = (*it)->get_url();
        vector<string>playlistsname = (*it)->get_playlists();
        
        outfile << title << ',' << artist << ',' << album << ',' << mv_url << ',';
        for (int i = 0; i < playlistsname.size(); i++) {
            outfile << playlistsname[i] << ':';
        }
        outfile << endl;
